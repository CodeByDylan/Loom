...
| `Loom.Entities` | `Id<TEntity>`, `Entity<TSelf>`, `AggregateRoot<TSelf>`, `IDomainEvent`, `IDomainEventHandler<TEvent>`. Identity, identity equality, domain event collection, and the contract a handler implements. | Persistence. Querying. Anything that *performs* dispatch — the contract lives here so a domain project can declare handlers; what calls them is `Loom.Persistence.EntityFrameworkCore`. Audit fields. Concurrency tokens. A `ValueObject` base — `sealed record` already does that. |
