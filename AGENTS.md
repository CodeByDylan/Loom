...
| `Loom.Entities` | `Id<TEntity>`, `Entity<TSelf>`, `AggregateRoot<TSelf>`, `IAggregateRoot`, `IDomainEvent`, `IDeferredDomainEvent`, `IDomainEventHandler<TEvent>`. Identity, identity equality, domain event collection, and the contract a handler implements. `IDeferredDomainEvent` marks an event as owed through the outbox rather than delivered in the save — the marker lives here because the event declares it; what honours it is `Loom.Persistence.EntityFrameworkCore`, which throws if one is raised with no outbox configured. `Id<TEntity>` serialises through `System.Text.Json` as a plain string holding the GUID — never as an object, including as a property name — and that is not configurable: an identity is one value on the wire, and a knob would let two services disagree about a shared contract. | Persistence. Querying. Anything that *performs* dispatch — the contract lives here so a domain project can declare handlers; what calls them is `Loom.Persistence.EntityFrameworkCore`. Audit fields. Concurrency tokens. A `ValueObject` base — `sealed record` already does that. |
| `Loom.Results.AspNetCore` | `ToHttpResult()` and `ToProblemDetails()`: the category-to-status-code mapping, as RFC 9457. | Any options type. What is forced is fixed; what is taste is reachable by building the problem details and changing it. |
...
| n/a | `Loom.Results.Analyzers`, `Loom.Templates` | Neither ships an assembly a consumer references — one is a build-time asset packed into `Loom.Results`, the other is `dotnet new` content — so neither sits in the reference graph the tiers describe. |
...
- **`Loom.Results.Error` and `Error<TMetadata>` are unsealed, as the documented exception to seal-by-default.** Consumers derive from it to declare domain errors, and deriving is what makes them convert implicitly to a `Result`. C# forbids user-defined conversions from an interface (CS0552), so an `IError` interface could not have offered that.
...
- **A package extending another shares that one's namespace.** `Loom.Results.AspNetCore` declares `namespace Loom.Results`, `Loom.Handlers.FluentValidation` and `Loom.Handlers.Abstractions` declare `namespace Loom.Handlers`, and every file in `Loom.Persistence.EntityFrameworkCore` declares `namespace Loom.Persistence`. Deliberate: one `using Loom.Results;` gives a consumer both `Result` and `ToHttpResult()`, which is the whole point of splitting the package rather than the vocabulary. The identifier names the coupling — ASP.NET Core, FluentValidation, EF Core — so the *package* a consumer installs still says what it drags in. **A new package that extends an existing one takes the existing namespace; one that introduces a new concept takes its own.**
- **The package identifier is `CodeByDylan.Loom.<Name>`; the namespace and assembly stay `Loom.<Name>`.** Deliberate, not an oversight — do not "fix" it. Only the identifier needs the prefix, since that is what nuget.org reserves, and `Loom.` is unreservable: a common word that identifies nobody, with eighteen packages under it owned by other authors. `PackageId` is derived once in `Directory.Build.props`, so a new package inherits it by being named `Loom.<Name>`.
...
- **Tagging is the human action (§2.7), and the only gate.** `release.yml` runs only for a tag someone pushed, and nothing after that waits for approval — the tag is the decision, so treat pushing one as the release itself rather than as a step before it. An agent prepares a release; a person causes one by tagging.
- The `nuget` environment scopes the trust policy; it does not hold a release back. Deployment protection rules, required reviewers among them, are unavailable on this repository's plan — so **do not describe the environment as an approval step**, here or in the workflow. They are free on public repositories, which is where a second gate would come from if one is wanted.
- **A release verifies before it packs, and checks the version before it pushes.** `scripts/check-release-version.sh` compares every produced package against the tag. MinVer is configured, not magical: a wrong prefix or a shallow clone yields `0.0.0-alpha.0.N` while everything else looks healthy, and a published version can never be replaced — so the wrong number would be burned for good. It reads the `<LoomVersion>` pin out of the templates package too, since no file name shows it: `0.2.1` through `0.3.1` shipped correctly named and pinned to `1.0.0`, which has never existed, so every solution scaffolded from them failed to restore.
...
- **After a release, bump `<LoomVersion>` in both templates to the version just published.** The in-repo pin must name a version that restores from nuget.org, so it lags one release by design — `release.yml` rewrites it to the tag at pack — but the lag is exactly one release only if someone moves it each time. CI compiles scaffolds against whatever it names.
